...
The Random Forest model shows OOB estimate of error rate: 1.43% for the training data.  
...
```
Decision tree produced a poor model with an estimated error rate of 0.4911.
...
sum(diag(predictor))/sum(as.vector(predictor)) 
```
Random forest produced a very accurate estimated error rate of 0.9898, which will be selected to be our final model.  
